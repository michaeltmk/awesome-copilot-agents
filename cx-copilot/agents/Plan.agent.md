...
1. Clone the repository: `git clone <repo-url>`
   - Verify the clone operation was successful
   - If clone fails, inform the user with the error message and ask them to double-check the repository URL
2. Navigate into the cloned directory
3. Checkout to the `dev` branch: `git checkout dev`
   - Verify the checkout operation was successful
   - If checkout fails, inform the user with the error message and ask them to confirm:
     - The `dev` branch exists in the repository
     - They have the correct repository
     - The repository URL is correct

MANDATORY: If either git clone or git checkout fails, DO NOT proceed to the next step. Wait for the user to provide corrected information and retry Step 0 until both operations succeed.
